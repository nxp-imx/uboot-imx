...
static void __iomem *get_clk_root_target(enum clk_slice_type slice_type, u32 slice_index)
{
	void __iomem *clk_root_target;

	switch (slice_type) {
	case CORE_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_CORE_CLK_ROOT_GEN_TAGET(slice_index);
		break;
	case BUS_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_BUS_CLK_ROOT_GEN_TAGET(slice_index);
		break;
	case IP_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_IP_CLK_ROOT_GEN_TAGET(slice_index);
		break;
	case AHB_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_AHB_CLK_ROOT_GEN_TAGET(slice_index);
		break;
	case IPG_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_IPG_CLK_ROOT_GEN_TAGET(slice_index);
		break;
	case CORE_SEL_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_CORE_SEL_CLK_ROOT_GEN_TAGET;
		break;
	case DRAM_SEL_CLOCK_SLICE:
		clk_root_target = (void __iomem *)CCM_DRAM_SEL_CLK_ROOT_GEN_TAGET;
		break;
	default:
		return NULL;
	}
...
/* AUDO SLOW is ignored, they are rarely used. */
		   ...
		   enum root_post_div post_div, enum clk_root_src clock_src)
{
	...
	if (p->slice_type == CORE_CLOCK_SLICE) {
		if (post_div > CLK_ROOT_POST_DIV8) {
			printf("Error post div!\n");
			return -EINVAL;
		}
	}
	...
	if (p->slice_type == IPG_CLOCK_SLICE) {
		if (post_div > CLK_ROOT_POST_DIV4) {
			printf("Error post div!\n");
			return -EINVAL;
		}
	}
	...
	val = CLK_ROOT_ON | (pre_div << CLK_ROOT_PRE_DIV_SHIFT) |
	      ...
	      src_entry << CLK_ROOT_SRC_MUX_SHIFT;
...
int clock_enable(enum clk_ccgr_index index, bool enable)
{
	...
	if (enable)
		ccgr = (void __iomem *)CCM_CCGR_SET(index);
	else
		ccgr = (void __iomem *)CCM_CCGR_CLR(index);
