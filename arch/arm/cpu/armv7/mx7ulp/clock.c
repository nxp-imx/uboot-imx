...
{
	u32 val;
	/* Seems the OCOTP CLOCKs have been enabled at default, check its inuse flag */
	...
	if (!(val & PCC_INUSE_MASK))
		writel(PCC_CGC_MASK, (PCC1_RBASE + 4 * OCOTP_CTRL_HIGH4K_PCC1_SLOT));
}
#endif
...
static void init_clk_rgpio2p(void)
{
	...
	/* Hard code to enable RGPIO2P0 clock since it is not in clock frame for A7 domain */
...
void clock_init(void)
{
	 ...
	 *   A7 side:  SPLL PFD0 (scs selected, 413Mhz), APLL PFD0 (352Mhz), DDRCLK, all NIC clocks
	 *             A7 Plat0 (NIC0) = 176Mhz, Plat1 (NIC1) = 176Mhz, IP BUS (NIC1_BUS) = 58.6Mhz
	 *
	 *   In u-boot, we will:
	 *   1. Enable PFD1-3 of APLL for A7 side. Enable FIRC and DIVs.
	 *   2. Enable USB PLL
	 *   3. Init the clocks of peripherals used in u-boot but without set rate
	 *       interface.  The clocks for these peripherals are enabled in this intialization.
	 *   4. Other peripherals with set clock rate interface does not be set in this function.
	 */
