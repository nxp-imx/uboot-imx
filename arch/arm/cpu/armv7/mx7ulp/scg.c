...
static u32 scg_apll_get_rate(void)
{
	...
	if (!val) {
		...
		val = (reg & SCG_PLL_CFG_POSTDIV1_MASK) >> SCG_PLL_CFG_POSTDIV1_SHIFT;
		rate = rate / (val + 1);

		val = (reg & SCG_PLL_CFG_POSTDIV2_MASK) >> SCG_PLL_CFG_POSTDIV2_SHIFT;
		rate = rate / (val + 1);
	} else {
		/* APLL PFD clock */
		val = (reg & SCG_PLL_CFG_PFDSEL_MASK) >> SCG_PLL_CFG_PFDSEL_SHIFT;
...
int scg_enable_pll_pfd(enum scg_clk clk, u32 frac)
{
	...
	/* Un-gate the PFD (Need un-gate before checking valid, not align with RM) */
	reg &= ~gate;
	writel(reg, addr);
...
int scg_enable_usb_pll(bool usb_control)
{
	...
	if (!usb_control) {
		while (timeout--) {
			if (readl(&scg1_regs->upllcsr) & SCG_UPLL_CSR_UPLLVLD_MASK)
				break;
		}
...
#define SCG1_RCCR_CFG_MASK              (SCG_CCR_SCS_MASK       |   \
										SCG_CCR_DIVBUS_MASK)
/* A7 Plat clck = A7 Core Clock / 2 = 250MHz / 1 = 250MHz */
#define SCG1_RCCR_DIVBUS_NUM            ((0x1)  << SCG_CCR_DIVBUS_SHIFT)
#define SCG1_RCCR_CFG_NUM               (SCG1_RCCR_SCS_NUM      |   \
										SCG1_RCCR_DIVBUS_NUM)
...
void scg_a7_spll_init(void)
{
	...
	/* Per block guide, "When changing PFD values, it is recommneded PFDx clock
	* gets gated first by writing a value of 1 to PFDx_CLKGATE register, then
	* program the new PFD value, then poll the PFDx_VALID flag to set before
	* writing a value of 0 to PFDx_CLKGATE to ungate the PFDx clock and allow
	* PFDx clock to run"
	*/
...
/* A7 APLL = 24MHz / 1 * 22 / 1 / 1 = 528MHz, system PLL is sourced from APLL, APLL clock source is system OSC (24MHz) */
...
/* #define SCG1_APLL_CFG_NUM_24M_OSC       0x160000 */

#define SCG1_APLL_PFD0_FRAC_NUM         (27)   /* PFD0 Freq = A7 APLL(528MHz) * 18 / 27 = 352MHz */
...
void scg_a7_soscdiv_init(void)
{
	...
	/* Configure A7 FIRC DIV1 ~ DIV3 */
	writel((SCG1_SOSCDIV_DIV3_NUM |
			SCG1_SOSCDIV_DIV2_NUM |
