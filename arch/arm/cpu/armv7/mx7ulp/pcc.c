...
int pcc_clock_sel(enum pcc_clk clk, enum scg_clk src)
{
	...
	if (!(val & PCC_PR_MASK) || (val & PCC_INUSE_MASK) || (val & PCC_CGC_MASK)) {
...
int pcc_clock_div_config(enum pcc_clk clk, bool frac, u8 div)
{
	...
	if (clk >= ARRAY_SIZE(pcc_arrays) || div > 8 || (div == 1 && frac != false))
		return -EINVAL;
...
u32 pcc_clock_get_rate(enum pcc_clk clk)
{
	...
	if (pcc_arrays[clk].div == PCC_HAS_DIV) {
		...
		/* Theoretically don't have overflow in the calc, the rate won't exceed 2G */
