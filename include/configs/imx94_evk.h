...
#define JAILHOUSE_ENV \
	"jh_mmcboot=setenv fdtfile imx94-19x19-evk-root.dtb; " \
		    "setenv jh_clk clk_ignore_unused mem=1280MB kvm-arm.mode=nvhe; " \
		    "if run loadimage; then run mmcboot;" \
		    "else run jh_netboot; fi; \0" \
	"jh_netboot=setenv fdtfile imx94-19x19-evk-root.dtb; " \
...
#define CFG_EXTRA_ENV_SETTINGS		\
	...
	BOOTENV \
	AHAB_ENV \
	"scriptaddr=0x99500000\0" \
	"kernel_addr_r=0x90800000\0" \
	"image=Image\0" \
	...
	"mmcautodetect=yes\0" \
	"mmcargs=setenv bootargs ${jh_clk} console=${console} root=${mmcroot} cpuidle.off=1\0 " \
	...
	"netargs=setenv bootargs ${jh_clk} console=${console} " \
		"root=/dev/nfs " \
		"ip=dhcp nfsroot=${serverip}:${nfsroot},v3,tcp cpuidle.off=1\0" \
